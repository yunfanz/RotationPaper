...
Visibility-based pipelines are not fundamentally limited to using only fully redundant baselines. In fact, most sensitivity forecasts to date do include partial redundancy \citep{Pobersens, HERA, JoshAntPos}. \textbf{A number of methods to produce power spectrum measurements using partially redundant baselines had been studied in recent years. A power-spectrum estimator based on visibilities gridded in the $uv$-plane was introduced in \cite{Choudhury2014Estimators, Choudhury2016Taperingb, Choudhury2016Taperinga}, and compared to a simple pair-baseline estimator in \cite{Choudhury2014Estimators}. Due to considerations of computational complexity, the authors favored grid-based estimator over pair-baseline estimators despite the higher accuracy of the latter.  \cite{wterm} proposed another approach based on gridded visibilities in sky-tracking measurements. Arrays capable of tracking include MWA and LOFAR, but not drift-scan telescopes such as PAPER, HERA, and potentially SKA-low. \cite{CTrott} studied three observing strategies (tracking, zenith drift scan, and drift and shift) with a pair-visibility based approach and concluded that zenith drift scan was the preferred observing strategy. In this paper, we introduce a pair-baseline power spectrum estimator that can be applied to both tracking-capable arrays and drift scan only arrays. Our formalism also provides a way to identify the pairs to cross-correlate, overcoming the previously considered computational disadvantage as compared to grid-based estimators.
Furthermore, compared to the pair-baseline estimators studied in \cite{Choudhury2014Estimators} and \cite{CTrott}, our analysis of the sensitivities indicate the importance of the three dimensional nature of the sky, resulting in us directly addressing curved sky effects in our formalism.  }
