...
Other observational studies of reionization include for example Gunn-Peterson measurements of quasi-stellar objects \citep{Fan2006}. GP tests rely on Ly$\alpha$ absorption, and are thus insensitive to the degree of ionization and can only probe the end stage of reionization. The 21cm line can in comparison directly probe the
...
One of the main challenges of observations of the cosmic
...
foreground contamination in power spectrum measurements. In the first, sometimes called  ``foreground
removal" technique, individual sources are identified and removed in the image domain.
The other technique, commonly referred to as ``foreground avoidance'', form power-spectrum directly from the visibilities, an arguably more natural approach from an instrumental point of view.  
Earlier studies have shown that the Fourier signature of smooth-spectrum foreground contaminants are constrained to a ``foreground
wedge'' \citep{wedge1,wedge2}. Contamination can thus be avoided
...
challenge of using the avoidance technique is thus sensitivity. This is the motivation for the design of
...
focus on repeated measurements of the same Fourier mode
with regular grids of antennas. Since baselines
of the same length and orientation measure the same Fourier modes
on the sky, a maximum redundancy array increases the
...
measurements with the 64-element version of PAPER (henceforth referred to as PAPER-64). 

While an imaging based power-spectrum pipeline naturally includes all redundancy information, visibility-based pipelines until now only cross multiply simultaneously redundant baselines. Here we present a method to
improve the sensitivity output of the visibility-based power-spectrum pipelines by including pairs of partially redundant baselines. To extract maximal partial redundancy we make use of the earth's rotation. 
The earth's rotation causes the baselines in a drift-scan array to pick up different modes of the sky with time. In a maximally redundant array, we use the fact that baselines that are slightly different
rotate into each other at a time delay to extract power-spectrum from their respective visibilities.  Our contribution is twofold. First we introduce a formalism to include pairs of partially-redundant baselines. Secondly we show how to use the formalism to automaticaly preselect baseline pairs and time offsets, making the problem computationally efficient. Recently, \cite{wterm} proposed an algorithmic approach to extract information from partially redundant baselines, by carefully treating the $w$-term in the fringe pattern to obtain weights for cross-correlation. The algorithm makes use of a time dependent coordinate system (``moving grid'') and is thus computationally expensive. We here present a formalism that allows us to compute such weights by simply performing a numerical integral. This allows us to identify good baselines to cross-correlate and predict the weight of sensitivity with speed and accuracy, thereby making the power spectrum estimation from a large number of baseline pairs computationally tractable. By using pairs of baselines, we also reduce additional mode mixing. As discussed in \cite{Hazelton2013}, the varying shapes of overlapping regions in $uv$ space of all contributing baselines lead to chromaticities in the response. By considering only pairs of baselines, we fix shapes of overlapping regions, and only let their sizes vary with frequency, just as in the case of fully redundant baselines.  
...
times. Non-equivalent baselines can also be partially redundant, with their partial redundancy maximized if their
...
2 we introduce the formalism for weighting partially redundant/near-equivalent baselines.
...
equivalent. In the bottom panel, the two baselines (5,77) and (40,85) are an example of equivalent pair, with separation denoted by \{1, 0\}, for the
antennas are separated by 1 unit east and 0 unit north. Similarly,
the baselines (5,69) and (5,85) are examples
of \{1, 1\} and \{1,-1\}, respectively. Note \{1, 0\} and \{-1, 0\} for example
...
Note \{1, 0\} and \{-1, 0\} for example are the same baselines and should
...
equivalent to 
\{2, 0\}, \{2, 1\} and \{2, -1\} \cite{Ali2015} of PAPER-128. There each of these classes
